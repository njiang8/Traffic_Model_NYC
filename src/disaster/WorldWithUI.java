...
public class WorldWithUI extends GUIState
{
    ...
    private GeomVectorFieldPortrayal bldgPortrayal = new GeomVectorFieldPortrayal();
    ...
    // Buildings
    final Color bldg = new Color (82,82,82);  		// dark gray;
    ...
    public void init(Controller c) {
        ...
        display.attach(bldgPortrayal, "Buildings");
    ...
    private void setupPortrayals() {
        ...
        bldgPortrayal.setField(world.bldgField);
        bldgPortrayal.setPortrayalForAll(new bldgGeomPortrayal());
    ...
    private void setupCharts() {
    	...
    	state.schedule.scheduleRepeating(new Steppable()  {
    		...
    		public void step (SimState state) {
            	...
            	indvsNotWorking.add(steps, Monitors.stayAtHome, true);
            	indvsToSchoolDaycare.add(steps, Monitors.toSchoolDaycare, true);
            	indvsAtHome.add(steps, Monitors.atHomeCount, true);
            	indvsCommuting.add(steps, Monitors.onCommuteCount, true);
            	indvsAtWork.add(steps, Monitors.atWorkCount, true);
            	indvsDead.add(steps, Monitors.indvDeaths, true);
            	indvsFleeingHome.add(steps,Monitors.affectedHeadedHome, true);
            	indvsResponding.add(steps,Monitors.inZoneFirstResponders, true);
            	indvsBlocked.add(steps,Monitors.agentsBlocked, true);
            	
            	indvsHCat0.add(steps, Monitors.healthCat[0], true);
            	indvsHCat1.add(steps, Monitors.healthCat[1], true);
            	indvsHCat2.add(steps, Monitors.healthCat[2], true);
            	indvsHCat3.add(steps, Monitors.healthCat[3], true);
            	indvsHCat4.add(steps, Monitors.healthCat[4], true);
            	indvsHCat5.add(steps, Monitors.healthCat[5], true);
            	indvsHCat6.add(steps, Monitors.healthCat[6], true);
            	indvsHCat7.add(steps, Monitors.healthCat[7], true);
            	indvsHCat8.add(steps, Monitors.healthCat[8], true);
            	indvsHCat9.add(steps, Monitors.healthCat[9], true);
            	indvsHCat10.add(steps, Monitors.healthCat[10], true); 
            	...
            	if ( (steps % 30 == 0) ) //> Parameters.tDetonation ) 
        		   System.out.println("WorldWithUI>step>" 
        				   + " " + step 
        				   + " " + Monitors.agentpopulation
        				   + " " + Monitors.stayAtHome
        				   + " " + Monitors.toSchoolDaycare
    ...
    public class indvGeomPortrayal extends GeomPortrayal {
    		...
    		public Paint determineColor(Object object) {		
	    		...
	    		int hColor = (int) ((Indv) ((MasonGeometry)object).getUserData()).getHealthStatus();
...
//    			scale = 0.003; // for entire region
    ...
    private ShapePortrayal2D getBuildingPortrayal() {
    	// trying to build another building agent shape
    	return new ShapePortrayal2D(ShapePortrayal2D.X_POINTS_SQUARE,ShapePortrayal2D.Y_POINTS_SQUARE) {
    		
    		public Paint determineColor(Object object) {		
    			// determines color bldg Type
    			int bColor = (int) ((MasonGeometry)object).getUserData();
    			
    			// System.out.println("WorldWithUI->bldgGeomPortrayal->determineColor>Type: " + bColor);
    			if (bColor == 1)  		{ return home; } // assume healthy first...to reduce computation
    			else if (bColor == 2)	{ return work; }
    			else if (bColor == 3)	{ return school; }
    			else if (bColor == 4)  	{ return dCare; } 
    			else return bldg; // default to grey
    		}
    		
    		public void draw (Object object, Graphics2D graphics, DrawInfo2D info) {
    			scale = 0.0004; // set scale of the portrayal for commuter region
    			paint = determineColor(object);
    			super.draw(object, graphics, info);
    		}
    	};
    ...
    public class bldgGeomPortrayal extends GeomPortrayal {
    	// trying to build another building agent shape
		public Paint determineColor(Object object) {		
			// determines color bldg Type
			int bColor = (int) ((MasonGeometry)object).getUserData();
		...
		public void draw (Object object, Graphics2D graphics, DrawInfo2D info) {
			scale = 0.0004; // set scale of the portrayal
			paint = determineColor(object);
			super.draw(object, graphics, info);
		} 	
    }
